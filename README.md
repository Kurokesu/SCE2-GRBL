...
Python IntelHex package is used by cmake for merging bootloader and application hex files. Python virtual environment is initialised upon project config within `build` directory. `intelhex` is run from this environment by cmake. For it to work make sure to install `python`, `python-venv` and `pip`.
...
#### Available STM32 flash tools

Can be selected in [CMakePresets.json](./CMakePresets.json)
...
```bash
cmake -DCMAKE_BUILD_TYPE=Release -DTARGET_MCU=stm32f103xb -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DSTM32_FLASH_TOOL=st-flash -DCMAKE_TOOLCHAIN_FILE=./cmake/gcc-arm-none-eabi.cmake -S ./ -B ./build/stm32f103xb-release -G Ninja
```
